...
type CallScreenRouteProp = RouteProp<CallParamList, 'Call'>;
...
const CallScreen = () => {
  ...
  const callTimeout = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (isIncoming) {
      ...
      socket.emit("accept_call", { to: user.uid });
    }
    ...
    const handleWebRTCAnswer = async ({ sdp, from }: any) => {
